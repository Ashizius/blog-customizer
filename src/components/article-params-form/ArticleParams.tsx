...
import { ArticleStateType, AllStatesListType, OptionType } from "../../constants/articleProps";
...
export const ArticleParams = (props:TFullParameters) => {
	...
	useEffect(()=>{
		!isSimilar(current,articleOptions)?setArticleOptions({...current}):undefined;
	},[props.current])
	...
	<>
		<Select title= {'шрифт'}
				...
				onChange ={(option:OptionType)=>{changeOption('fontFamilyOption',option)}}
		/>
		<RadioGroup	title = {'размер шрифта'}
					options= {all.fontSizeOptions}
					selected={articleOptions.fontSizeOption}
					onChange ={(option:OptionType)=>{changeOption('fontSizeOption',option)}}
					name= {'fontSizeOption'}
		/>
		<Select title= {'цвет шрифта'}
				...
				onChange ={(option:OptionType)=>{changeOption('fontColor',option)}}
		/>
		<Separator/>
		<Select title= {'цвет фона'}
				...
				onChange ={(option:OptionType)=>{changeOption('backgroundColor',option)}}
		/>
		<Select title= {'ширина контента'}
				...
				onChange ={(option:OptionType)=>{changeOption('contentWidth',option)}}
		/>
